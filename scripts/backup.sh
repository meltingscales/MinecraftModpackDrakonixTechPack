...
server_process_running() {
    grep -zqs "^MC_SERVER_MARKER=drakonixtechpack$" /proc/[0-9]*/environ 2>/dev/null
}
...
# world/config - not in git, not reproducible from the pack. Only include
...
set +e
tar -C "$SERVER_DIR" -czf "$BACKUP_DIR/drakonixtechpack-world-$(date +%Y%m%d-%H%M%S).tar.gz" \
...
ls -1t "$BACKUP_DIR"/drakonixtechpack-world-*.tar.gz | tail -n "+$((RETAIN + 1))" | xargs -r rm -f
